<template>
  <Layout>
    <div class="flex gap-2 items-end">
      ...
      <span @click="resetBarberSelection" class="text-red-500 cursor-pointer"
        >Ripristina</span
      >
    </div>
    <div class="p-4 md:grid md:grid-cols-4 flex flex-col gap-4">
      <AppointmentCard
        v-for="value in filteredAppointments"
        :key="value.id"
        :name="value.clientName"
        :barber="value.barber.name"
        :date="`${new Date(value.date).toLocaleDateString()} ${value.dateTime}`"
        :service="value.service.name"
        @delete="setSelectedAppointment(value.id)"
      />
    </div>
    <div
      class="text-black font-semibold pl-4"
      v-if="!appointments.content.length || !filteredAppointments.length"
      aria-live="polite"
    >
      Nessun risultato trovato
    </div>
    <ConfirmModal
      :is-open="selectedAppointment !== null"
...
const filteredAppointments = computed(() => {
  ...
  return appointments.value.content.filter(
    (item) => item.barber.id === selectedBarber.value,
  );
});
